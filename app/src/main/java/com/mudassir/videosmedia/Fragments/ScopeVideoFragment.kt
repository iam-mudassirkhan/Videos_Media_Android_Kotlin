...
import com.mudassir.videosmedia.databinding.FragmentScopeVideoBinding


private const val READ_EXTERNAL_STORAGE_REQUEST = 1
private const val DELETE_PERMISSION_REQUEST = 2

class ScopeVideoFragment : Fragment() {


    private var binding: FragmentScopeVideoBinding? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentScopeVideoBinding.inflate(LayoutInflater.from(requireContext()), container, false)
        ...
        viewModel.videos.observe(requireActivity(), Observer<List<VideoModel>> { videos ->
            galleryAdapter.submitList(videos)
            Toast.makeText(requireContext(), "${videos.size}", Toast.LENGTH_SHORT).show()
        })
    ...
    private inner class GalleryAdapter(val onClick: (VideoModel) -> Unit) :
        ListAdapter<VideoModel, VideoViewHolder>(VideoModel.diffCallback) {
...
private class VideoViewHolder(view: View, onClick: (VideoModel) -> Unit) :
    ...
    init {
        imageView.setOnClickListener {
            val video = rootView.tag as? VideoModel ?: return@setOnClickListener
            onClick(video)
        }
    }
